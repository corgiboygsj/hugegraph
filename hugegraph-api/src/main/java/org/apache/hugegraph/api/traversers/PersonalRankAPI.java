...
public class PersonalRankAPI extends API {
    ...
    public String personalRank(@Context GraphManager manager,
        ...
        E.checkArgument(request.maxDegree > 0L || request.maxDegree == HugeTraverser.NO_LIMIT,
                        "The max degree of rank request must be > 0 " +
                        "or == -1, but got: %s", request.maxDegree);
        E.checkArgument(request.limit > 0L || request.limit == HugeTraverser.NO_LIMIT,
                        "The limit of rank request must be > 0 or == -1, " +
                        "but got: %s", request.limit);
        E.checkArgument(request.maxDepth > 0L &&
                        request.maxDepth <= Long.parseLong(
                                HugeTraverser.DEFAULT_MAX_DEPTH),
                        "The max depth of rank request must be " +
                        "in range (0, %s], but got '%s'",
                        HugeTraverser.DEFAULT_MAX_DEPTH, request.maxDepth);
    ...
    private static class RankRequest {
        ...
        private long maxDegree = Long.parseLong(HugeTraverser.DEFAULT_MAX_DEGREE);
        @JsonProperty("limit")
        private long limit = Long.parseLong(HugeTraverser.DEFAULT_LIMIT);
