...
public class BinarySerializer extends AbstractSerializer {
    ...
    protected void parseEdge(BackendEntry.BackendColumn col, HugeVertex vertex,
                             HugeGraph graph) {
        // owner-vertex + dir + edge-label + sort-values + other-vertex
    ...
    private Query writeQueryEdgeRangeCondition(ConditionQuery cq) {
        ...
        Condition.RangeConditions
                range = new Condition.RangeConditions(sortValues);
    ...
    private Query writeRangeIndexQuery(ConditionQuery query) {
        ...
        Condition.RangeConditions range = new Condition.RangeConditions(fields);
        if (range.keyEq() != null) {
    ...
    private final class SchemaSerializer {
        ...
        private <T extends SerialEnum> T readEnumOrDefault(HugeKeys key,
            ...
            BackendEntry.BackendColumn
                    column = this.entry.column(formatColumnName(key));
            if (column == null) {
                return defaultValue;
            }
