...
public class WhereBuilderTest extends BaseUnitTest {
    ...
    public void testRelation() {
        ...
        where.relation("key1", Condition.RelationType.EQ, "value1");
        ...
        where.and().relation("key2", Condition.RelationType.EQ, "value2");
        Assert.assertEquals(" key1='value1' AND key2='value2'",
                            where.toString());

        where = new WhereBuilder(false);
        where.relation("key1", Condition.RelationType.NEQ, "value1");
        Assert.assertEquals(" key1!='value1'", where.toString());

        where = new WhereBuilder(false);
        where.relation("key1", Condition.RelationType.GT, "value1");
        Assert.assertEquals(" key1>'value1'", where.toString());

        where = new WhereBuilder(false);
        where.relation("key1", Condition.RelationType.GTE, "value1");
        Assert.assertEquals(" key1>='value1'", where.toString());

        where = new WhereBuilder(false);
        where.relation("key1", Condition.RelationType.LT, "value1");
        Assert.assertEquals(" key1<'value1'", where.toString());

        where = new WhereBuilder(false);
        where.relation("key1", Condition.RelationType.LTE, "value1");
        Assert.assertEquals(" key1<='value1'", where.toString());

        where = new WhereBuilder(false);
        where.relation("key1", Condition.RelationType.IN, ImmutableList.of("v1", "v2"));
